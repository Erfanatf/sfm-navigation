"""DW4DO controller with unified maneuver manager."""
import time
import numpy as np
...
@njit
def _compute_time_to_collision(traj, obstacles, robot_radius, safety_margin, dt):
    total_clearance = robot_radius + safety_margin
    n_steps = traj.shape[0]
    for i in range(n_steps):
        t = i * dt
        robot_x, robot_y = traj[i, 0], traj[i, 1]
        for j in range(obstacles.shape[0]):
            obs_x = obstacles[j, 0] + obstacles[j, 3] * t
            obs_y = obstacles[j, 1] + obstacles[j, 4] * t
            obs_r = obstacles[j, 2]
            dist = np.sqrt((robot_x - obs_x) ** 2 + (robot_y - obs_y) ** 2)
            if dist < obs_r + total_clearance:
                return t
    return np.inf
...
def _dw4do_core(
    ...
    predict_time = n_predict_steps * dt
    ...
    for i in range(n_v):
        ...
        for j in range(n_omega):
            ...
            collision = False
            min_dist = np.inf
            for step in range(n_predict_steps + 1):
                t = step * dt
                tx, ty = traj[step, 0], traj[step, 1]
                for obs_idx in range(obstacles.shape[0]):
                    pred_x = obstacles[obs_idx, 0] + obstacles[obs_idx, 3] * t
                    pred_y = obstacles[obs_idx, 1] + obstacles[obs_idx, 4] * t
                    obs_r = obstacles[obs_idx, 2]
                    dist = (
                        np.sqrt((tx - pred_x) ** 2 + (ty - pred_y) ** 2)
                        - obs_r - robot_radius - safety_margin
                    )
                    if dist < 0:
                        collision = True
                        break
            ...
            ttc = _compute_time_to_collision(traj, obstacles, robot_radius, safety_margin, dt)
            ttc_score = min(ttc / predict_time, 1.0) if np.isfinite(ttc) else 1.0
...
class DW4DO(BaseController):
    ...
    def compute_velocity(
        ...
        # Convert obstacles for DWA core
        if isinstance(obstacles, list):
            if len(obstacles) == 0:
        ...
        best_v, best_omega, _, safe_count = _dw4do_core(
