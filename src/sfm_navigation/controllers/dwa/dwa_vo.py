"""DWA‑VO controller with unified maneuver manager."""
...
from .dwa_utils import compute_vo_cone, is_velocity_in_vo
from ..maneuvers import ManeuverDOB, ManeuverManager
from ..mpc.base_mpc import ControlLPF


@njit
def _compute_vo_penalty(
    robot_vx, robot_vy, robot_x, robot_y, robot_radius, obstacles, time_horizon
):
    penalty = 0.0
    for i in range(obstacles.shape[0]):
        ...
        obs_y = obstacles[i, 1]
        obs_r = obstacles[i, 2]
        obs_vx = obstacles[i, 3]
        obs_vy = obstacles[i, 4]
        ...
        if is_velocity_in_vo(
            robot_vx, robot_vy, apex_x, apex_y, left_angle, right_angle
        ):
            rel_vx = robot_vx - apex_x
            rel_vy = robot_vy - apex_y
...
def _dwa_vo_core(
    ...
    for i in range(n_v):
        ...
        for j in range(n_omega):
            ...
            rvx = v * np.cos(theta)
            rvy = v * np.sin(theta)
            vo_penalty = _compute_vo_penalty(
                rvx, rvy, x, y, robot_radius, obstacles, time_horizon
            )
...
class DWA_VO(BaseController):
    ...
    def compute_velocity(
        ...
        **kwargs
        ...
        best_v, best_omega, _, safe_count = _dwa_vo_core(
            x,
            y,
            theta,
            v_curr,
            ...
            self.config.v_resolution,
            self.config.w_resolution,
            dt,
            n_predict_steps,
            self.dwa_window_time,
