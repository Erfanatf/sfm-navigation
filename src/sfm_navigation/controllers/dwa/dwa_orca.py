"""DWA‑ORCA controller with unified maneuver manager."""
import time
import numpy as np
...
from .dwa_utils import compute_orca_halfplane, velocity_satisfies_orca
from ..maneuvers import ManeuverDOB, ManeuverManager
from ..mpc.base_mpc import ControlLPF


@njit
def _count_orca_violations(robot_vx, robot_vy, robot_x, robot_y, robot_radius, obstacles, time_horizon):
    violations = 0
    total_penalty = 0.0
    for i in range(obstacles.shape[0]):
        obs_x, obs_y, obs_r, obs_vx, obs_vy = obstacles[i, 0], obstacles[i, 1], obstacles[i, 2], obstacles[i, 3], obstacles[i, 4]
        point_x, point_y, normal_x, normal_y = compute_orca_halfplane(
            robot_x, robot_y, robot_radius, robot_vx, robot_vy,
            obs_x, obs_y, obs_r, obs_vx, obs_vy, time_horizon,
        )
        if normal_x != 0 or normal_y != 0:
            if not velocity_satisfies_orca(robot_vx, robot_vy, point_x, point_y, normal_x, normal_y):
                violations += 1
                diff_x = robot_vx - point_x
                diff_y = robot_vy - point_y
                depth = -(diff_x * normal_x + diff_y * normal_y)
                total_penalty += max(0.0, depth) + 0.5
...
def _dwa_orca_core(
    ...
    time_horizon, alpha, beta, gamma, lambda_orca,
):
    ...
    for i in range(n_v):
        ...
        for j in range(n_omega):
            ...
            rvx = v * np.cos(theta)
            rvy = v * np.sin(theta)
            violations, orca_penalty = _count_orca_violations(rvx, rvy, x, y, robot_radius, obstacles, time_horizon)
...
class DWA_ORCA(BaseController):
    ...
    def compute_velocity(
        ...
        best_v, best_omega, _, safe_count = _dwa_orca_core(
            ...
            self.time_horizon, self.alpha, self.beta, self.gamma, self.lambda_orca,
        )
